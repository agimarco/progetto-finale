...
if args.record:
    env = wrappers.Monitor(env, 'videos/cnn_video_'+args.details, force=True, video_callable=lambda episode_id: True)
env = NoiseWrapper(env)
env = DiscreteActionWrapperTrain(env)
env = DtRewardWrapper(env)
env = ResizeWrapper(env)

model = DDQN(env, activation='relu', cnn=True, mlp_width=64)
...
def update(dt):
    ...
    global pause
    if(not pause):
        action, q_value = model.predict(obs, args.details == "Model_5")
